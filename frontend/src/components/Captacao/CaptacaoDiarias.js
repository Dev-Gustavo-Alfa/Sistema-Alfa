...
const CaptacaoDiaria = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:3001/captacao", formData);
  ...
  return (
    <div className="captacao-container">
      <h2>Captação Diária</h2>
        ...
        {formData.status === "captado" && (
          <>
            ...
            >
              ...
              <option value="Bhaia">BA</option>
              <option value="Belo Horizonte">BH</option>
        ...
        {(formData.status.length > 1 && formData.status != 'captado') && (
        ...
        <label>
          ...
          <input type="tel" name="celular" value={formData.celular} onChange={handleChange} placeholder="Celular" />
        </label>
